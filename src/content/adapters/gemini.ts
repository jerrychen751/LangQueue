...
class GeminiAdapter extends Adapter {
  ...
  getInputElement(): HTMLTextAreaElement | null {
    ...
    if (visibleCEs.length === 1) {
      ...
      if (match) {
        return match as unknown as HTMLTextAreaElement;
      }
    }
  ...
  isGenerating(): boolean {
    ...
    if (document.querySelector('.send-button.stop')) {
      return true;
    }

    // Response footer without .complete means still generating
    if (document.querySelector('.response-footer:not(.complete)')) {
      return true;
    }

    // Animated footer on the actively streaming response
    if (document.querySelector('.response-footer.animated')) {
      return true;
    }
  ...
  async attachFiles(files: File[]) {
    ...
    if (!composer || area?.localName !== 'input-area-v2') {
      return failure;
    }
    if (input) {
      return this.assignUploadFiles(input, files, composer, area);
    }
    const localScope = composer.closest('form') || composer.parentElement;
    if (localScope?.querySelectorAll('input[type="file"]').length) {
      return failure;
    }
    const buttons = area.querySelectorAll('button[aria-label="Upload & tools"][aria-haspopup="menu"]');
    if (buttons.length !== 1 || !isButtonEnabledAndVisible(buttons[0])) {
      return failure;
    }
    const button = buttons[0];
    const href = location.href;
    if (!composer.isConnected || !area.isConnected) {
      return failure;
    }
    if (button.getAttribute('aria-expanded') !== 'true') {
      if (Array.from(document.querySelectorAll('[role="menu"]')).some(isVisible)) {
        return failure;
      }
      button.click();
    }
    ...
    while (Date.now() - started < 1500) {
      if (location.href !== href || !composer.isConnected || this.getInputElement() !== composer || composer.closest('input-area-v2') !== area || !button.isConnected) {
        return failure;
      }
      ...
      if (menus.some(menu => !outer.includes(menu) && !nested.includes(menu)) || outer.length > 1 || nested.length > 1) {
        return failure;
      }
      if (outer.length === 1 && nested.length === 1) {
        if (!outer[0].contains(nested[0]) || button.getAttribute('aria-expanded') !== 'true') {
          return failure;
        }
        ...
        if (uploaders.length === 1) {
          return isFileInputEnabled(uploaders[0]) ? this.assignUploadFiles(uploaders[0], files, composer, area) : failure;
        }
      }
  ...
  private assignUploadFiles(input: HTMLInputElement, files: File[], composer: HTMLTextAreaElement, area: Element) {
    if (!composer.isConnected || !area.isConnected || this.getInputElement() !== composer) {
      return { ok: false, error: 'The chat composer changed before attachment upload.' };
    }
    const counts = new Map<string, number>();
    for (const preview of area.querySelectorAll('uploader-file-preview')) {
      const name = this.getUploadFilename(preview);
      if (!name) {
        return { ok: false, error: 'An existing attachment is not ready to identify. Finish or remove it before uploading another prompt.' };
      }
  ...
  async waitForUploadsComplete(options?: { timeoutMs?: number; pollMs?: number; files?: File[] }): Promise<boolean> {
    ...
    if (!attempt || !files?.length || files.length !== attempt.files.length || files.some((file, index) => file !== attempt.files[index])) {
      return false;
    }
    const expected = new Map(attempt.counts);
    for (const file of files) {
      expected.set(file.name, (expected.get(file.name) || 0) + 1);
    }
    const started = Date.now();
