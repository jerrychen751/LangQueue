...
export function createQueue(adapter: Adapter, getInput: () => InputElement | null, coordinator = createExecutionCoordinator()) {
  ...
  async function flush() {
    if (running || !items.length || status === 'failed' || status === 'cancelled') {
      return;
    }
    ...
    try {
      while (items.length) {
        ...
        await executeStep(adapter, getInput, item, 'overwrite', controller.signal, next => {
          ...
          if (next === 'sending') {
            item.preparedContent = item.content;
          }
          publish();
      ...
      if (error === 'SEND_FAILED' && items[0]) {
        items[0].sent = false;
      }
      if (controller.signal.aborted && !changedConversation && !items[0]?.sent) {
        const cancelledItem = items.shift();
        if (cancelledItem?.attachmentAttempted) {
          error = 'QUEUE_CANCELLED_ATTACHMENTS';
        }
      }
      if (items[0]?.attachmentAttempted && !items[0].sent) {
        error += ' Check and clear attachments in the composer before removing this item and adding it again.';
      }
    } finally {
      running = false;
      controller = null;
