...
export function assertExecutionContext(signal: AbortSignal, href: string) {
  ...
  if (getConversationHref() !== href) {
    throw new Error('CONVERSATION_CHANGED');
  }
  if (!isConversationReady(href)) {
...
export async function sendPromptWhenReady(
  ...
  function assertSendContext() {
    ...
    if (!input.isConnected || adapter.getInputElement() !== input) {
      throw new Error('COMPOSER_CHANGED: the chat input changed. Check your draft before retrying.');
    }
    if (getInputText(input) !== expectedText) {
      throw new Error('COMPOSER_CHANGED: save or clear your draft, then retry.');
    }
    if (adapter.isGenerating()) {
      throw new Error('The model started generating a response. Check the conversation before sending again.');
    }
  }
  while (true) {
    assertSendContext();
...
export async function executeStep(
  ...
  while (adapter.isGenerating() || !isInputReady(adapter.getInputElement())) {
    assertExecutionContext(signal, href);
    if (Date.now() - startedAt >= timing.timeoutMs) {
      throw new Error('MODEL_IDLE_TIMEOUT');
    }
    await waitForExecutionDelay(timing.pollMs, signal);
  }
  assertExecutionContext(signal, href);
  if (item.attachments?.length) {
    ...
    if (!attached.ok) {
      throw new Error(attached.error || 'ATTACHMENT_UPLOAD_FAILED');
    }
  ...
  if (mode === 'append') {
    appendInputText(input, item.content);
  } else {
  ...
  while (!adapter.isGenerating()) {
    assertExecutionContext(signal, href);
    if (Date.now() - startedAt >= timing.startTimeoutMs) {
