...
export function initController(adapter: Adapter) {
  ...
  const overlay = createOverlay({
    onSelect: (item) => {
      ...
      void (async () => {
        ...
        if (chainVersion !== null && chainVersion !== chainExecutor.getCancellationVersion()) {
          return;
        }
  ...
  function getEventInput(event: Event): InputElement | null {
    ...
    if (input instanceof HTMLElement && input.contains(target)) {
      return input;
    }
    return null;
  }
  ...
  function handleKeydown(event: KeyboardEvent) {
    ...
    if (event.key === 'Enter' && !event.shiftKey && !event.isComposing) {
      ...
      void ensureSettingsLoaded().then((ready) => {
        ...
        if (queueVersion !== queue.getCancellationVersion()) {
          return;
        }
  ...
  function isInputActive(): boolean {
    ...
    if (input instanceof HTMLElement && input.contains(active)) {
      return true;
    }
    return false;
  }
  ...
  async function handleInsertRequest({ content, attachments = [], expectedHref }: TabRequests['INJECT_PROMPT']['payload'], shouldSend: boolean): Promise<InsertPromptResult> {
    if (!content && attachments.length === 0) {
      return { ok: false, sendAttempted: false, reason: 'No prompt text or attachments were provided.' };
    }
    const expectedInput = adapter.getInputElement();
    const expectedText = expectedInput ? getInputText(expectedInput) : null;
    if (!await ensureSettingsLoaded()) {
      return { ok: false, sendAttempted: false, reason: 'Settings are unavailable. Try the action again to reload settings.' };
    }
    const input = adapter.getInputElement();
    if (input !== expectedInput || (input && getInputText(input) !== expectedText)) {
      return { ok: false, sendAttempted: false, reason: 'The draft changed while settings loaded. Try again when ready.' };
    }
  ...
  const requestHandlers: RequestHandlers<TabRequests> = {
    ...
    RUN_CHAIN: async ({ steps, expectedHref, insertionModeOverride }) => {
      ...
      if (coordinator.isBusy()) {
        return { ok: false, reason: 'COMPOSER_BUSY' };
      }
      void chainExecutor.run(steps, settings, insertionModeOverride);
      return { ok: true };
    },
