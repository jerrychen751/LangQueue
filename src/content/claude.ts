(() => {
  // Local types to avoid import issues in content scripts
  type ChainStep = {
    content: string
    autoSend?: boolean
  ...
  function findInput(): HTMLTextAreaElement | HTMLElement | null {
    // Claude commonly uses a contenteditable div with role textbox
    const selectors = [
  ...
  function isGeneratingNow(): boolean {
    ...
    // Heuristic: disabled send while text present can indicate generating
