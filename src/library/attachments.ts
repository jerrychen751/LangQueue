...
export function isAllowedAttachment(file: Pick<File, 'type' | 'name'>): boolean {
  if (matchesAllowedMime(file.type || '')) {
    return true;
  }
...
export function validateAttachmentFile(file: Pick<File, 'size' | 'type' | 'name'>): string | null {
  ...
  if (file.size > MAX_ATTACHMENT_BYTES) {
    return 'Attachment exceeds 25 MB limit.';
  }
...
export function createAttachmentDraft(file: File): AttachmentRef {
  ...
  if (validation) {
    throw new Error(file.name + ': ' + validation);
  }
...
export function prepareAttachmentImports(records: unknown): { ref: AttachmentRef; file: File }[] {
  if (!Array.isArray(records)) {
    throw new Error('Invalid attachment list in backup.');
  }
  const ids = new Set<string>();
  return records.map((raw) => {
    if (!raw || typeof raw !== 'object') {
      throw new Error('Invalid attachment record in backup.');
    }
    ...
    ids.add(record.id);
    if (record.size > MAX_ATTACHMENT_BYTES || record.dataBase64.length > Math.ceil(MAX_ATTACHMENT_BYTES / 3) * 4) {
      throw new Error('Backup attachment exceeds 25 MB limit.');
    }
    ...
    if (bytes.byteLength !== record.size) {
      throw new Error('Attachment size does not match its contents: ' + record.name);
    }
    if (record.kind !== undefined && record.kind !== 'image' && record.kind !== 'file') {
      throw new Error('Invalid attachment kind: ' + record.name);
    }
    const file = new File([bytes], record.name, { type: record.mimeType });
    if (file.type !== record.mimeType) {
      throw new Error('Attachment MIME type must use its canonical lowercase form: ' + record.name);
    }
    const kind = inferAttachmentKind(file.type);
    if (record.kind !== undefined && record.kind !== kind) {
      throw new Error('Attachment kind does not match its MIME type: ' + record.name);
    }
    const validation = validateAttachmentFile(file);
    if (validation) {
      throw new Error(record.name + ': ' + validation);
    }
    return {
