...
function createFixture(context) {
  ...
  const area = {
    ...
    querySelectorAll(selector) {
      if (selector === 'uploader-file-preview') {
        return fixture.previews;
      }
      if (selector.startsWith('.gem-attachment-loading-error')) {
  ...
  function addPreview(name, kind = 'document') {
    ...
    const preview = {
      ...
      querySelectorAll(selector) {
        if (selector.startsWith('gem-attachment ')) {
...
for (const scenario of ['no previews', 'partial', 'preexisting', 'duplicate requested', 'hidden', 'unloaded image', 'send disabled', 'busy', 'wrong filename']) {
  test(`Gemini does not complete uploads with ${scenario}`, async context => {
    ...
    if (scenario === 'partial') {
      files.push(new File(['second'], 'second.txt'));
    }
    if (scenario === 'duplicate requested') {
      files.push(new File(['second'], 'test.txt'));
    }
    ...
    if (!['no previews', 'preexisting'].includes(scenario)) {
      ...
      if (scenario === 'unloaded image') {
        image.complete = false;
      }
    }
    if (scenario === 'send disabled') {
      fixture.send = null;
    }
    if (scenario === 'busy') {
...
for (const scenario of ['navigation', 'composer replacement', 'area replacement', 'disconnected', 'superseded']) {
  test(`Gemini stops checking uploads after ${scenario}`, async context => {
    ...
    if (scenario === 'navigation') {
      globalThis.location.href += '-new';
    }
    if (scenario === 'composer replacement') {
      fixture.adapter.getInputElement = () => ({});
    }
    if (scenario === 'area replacement') {
      fixture.composer.closest = () => ({});
    }
    if (scenario === 'disconnected') {
      fixture.area.isConnected = false;
    }
