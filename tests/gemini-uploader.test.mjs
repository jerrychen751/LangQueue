...
for (const scenario of ['unrelated menu', 'duplicate uploader', 'detached menu', 'navigation', 'composer replacement', 'disabled local']) {
  test(`Gemini rejects ${scenario} without assigning files`, async () => {
    ...
    if (scenario === 'unrelated menu') {
      fixture.menus = [{ offsetParent: {}, getAttribute: () => 'Other menu' }];
    }
    if (scenario === 'duplicate uploader') {
      fixture.uploaders.push(new FileInput());
    }
    if (scenario === 'detached menu') {
      fixture.outer.contains = () => false;
    }
    if (scenario === 'navigation') {
      fixture.button.onClick = () => { open(); globalThis.location.href += '-other'; };
    }
    if (scenario === 'composer replacement') {
      fixture.button.onClick = () => { open(); fixture.adapter.getInputElement = () => ({}); };
    }
