...
test('prompt confirmation waits for deletion and ignores duplicate attempts', async () => {
  ...
  const confirmation = popup.find(popup.render(props), node => node.props.title === 'Delete this prompt?');
  const deleting = confirmation.props.onConfirm();
...
test('chain deletion waits for storage, prevents duplicate work, and reports failure', async () => {
  ...
  const confirmation = popup.find(popup.render(), node => node.props.title === 'Delete chain?');
  const deleting = confirmation.props.onConfirm();
  await confirmation.props.onConfirm();
  confirmation.props.onCancel();
  assert.equal(deletes, 1);
